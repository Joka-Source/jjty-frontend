...
const report={inputRoute:direct?'generated-owned-stream':'fake-device-owned-stream',browser:await browser.version(),syntheticAudioOnly:true,events:[]};
...
try{
 ...
 await page.locator('#voice-toggle').click();
 if(direct){assert.equal(await page.evaluate(()=>window.syntheticCaptureCount),1);assert.equal(await page.evaluate(()=>window.syntheticSpeechTrack.readyState),'ended');log('ownedCapture',{acquisitions:1,releasedOnPause:true});}
