...
export async function renderTender(surface){
 ...
 function draw(){
  ...
  <section class="tender-ai" aria-labelledby="tender-ai-title" data-service-error="${esc(aiState.error||'')}"><div class="tender-ai-heading"><div><div class="tender-ai-kicker"><span class="runner-dot" aria-hidden="true"></span><strong id="tender-ai-service-state">${aiState.kind==='ready'?'Intelligence service ready':aiState.kind==='running'?'Reviewing documents':aiState.kind==='error'?'Service unavailable':'Checking intelligence service'}</strong><span>Open-source document intelligence</span></div><h3 id="tender-ai-title">AI document review</h3><p>JJTY sends supported added documents to your configured service only when you request a review. Every finding must quote an exact source page and remains advisory until you check it.</p><small>${reviewableDocuments.length} document${reviewableDocuments.length===1?'':'s'} available for review${documents.length>reviewableDocuments.length?' · legacy BOQ and archives stay in the workspace':''}</small></div><button id="tender-ai-review" class="primary" ${aiState.kind!=='ready'||!reviewableDocuments.length?'disabled':''}>${aiState.kind==='running'?'Reviewing…':currentAnalysis?'Review again':'Review added documents'}</button></div>${analysisMarkup}<p class="tender-ai-boundary">AI cannot mark documents checked, confirm eligibility, price the BOQ, use the DSC, pay, or submit.</p></section>
