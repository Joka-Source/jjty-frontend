...
function communication(surface) {
 ...
 async function showAttachment(file) {
  ...
  if(file.type==='application/pdf'||/\.pdf$/i.test(file.name)){
   ...
   edit.onclick=async()=>{edit.disabled=true;try{const id=await startPdfHandoff(attachmentKey,file);location.href=`/reader/?workspaceSession=${id}#/home`;}catch(error){status(error.message);edit.disabled=false;}};node.append(edit);
  }
