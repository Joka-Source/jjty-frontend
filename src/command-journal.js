...
const STAGES=['capture','heard','intent','target','result'],RATINGS=['worked','missed','not-rated'];
...
export function createCommandJournal(options={}){
 ...
 const api={
  begin({source='voice',rawText,stage='heard',captureId}={}){const startedAt=time();let suffix;try{suffix=globalThis.crypto.randomUUID();}catch{suffix=Math.random().toString(36).slice(2)+'-'+(++sequence);}
   const id='cmd-'+suffix,entry={id,source:pick(source,['voice','pointer','sim'],'pointer'),startedAt,events:[cleanEvent({stage:stage==='capture'?'capture':'heard',status:stage==='capture'?'pending':'received',at:startedAt,rawText})],feedback:{rating:'not-rated'}};if(typeof captureId==='string'&&items.some(item=>item.id===captureId&&item.events.some(event=>event.stage==='capture')))entry.captureId=captureId;items.push(entry);items=items.slice(-limit);publish();return id;},
