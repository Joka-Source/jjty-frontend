...
export function pdfSelectionTarget(selection, doc, root) {
  ...
  function endpoint(node,offset) {
    ...
    const item=element?.closest('[data-pdf-char-start]');
    ...
    const start=Number(item.dataset.pdfCharStart),end=Number(item.dataset.pdfCharEnd);
  ...
  let previous=null;
  for(const item of root.querySelectorAll('[data-pdf-char-start]')){
