...
import { DraftBlock } from "./result.js";
...
const BLOCK_END: Record<string, DraftBlock["kind"]> = {
  p: "paragraph",
  div: "paragraph",
  section: "paragraph",
  article: "paragraph",
 ...
 */
export function htmlToBlocks(html: string): { drafts: DraftBlock[]; title?: string } {
  ...
  let current = "";
  let currentKind: DraftBlock["kind"] = "paragraph";
  let currentLocator: string | undefined;
  let dropDepthTag: string | null = null;
  let dropDepth = 0;
  let inPre = false;
  ...
  const flush = () => {
    ...
    if (text) {
      const d: DraftBlock = { text, kind: currentKind };
      if (currentLocator) d.locator = currentLocator;
      drafts.push(d);
    }
    current = "";
  ...
  while ((m = tagRe.exec(html)) !== null) {
    ...
    if (dropDepthTag !== null) {
      if (name === dropDepthTag) {
        if (isClose) {
          dropDepth--;
          if (dropDepth <= 0) dropDepthTag = null;
        } else if (!isSelfClose) {
          dropDepth++;
        }
      }
    ...
    if (kind) {
      if (!isClose) {
        ...
        flush();
        if (/^h[1-6]$/.test(name)) currentLocator = name;
      } else {
        currentKind = kind;
        if (/^h[1-6]$/.test(name)) currentLocator = name;
        flush();
