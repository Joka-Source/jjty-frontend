...
export function initAnnotationToolbar({currentDoc,root,run,review,canUndo,getColor=()=>'yellow',setColor=()=>{}}) {
  ...
  toolbar.innerHTML='<div class="annotation-actions"><button type="button" data-annotation="highlight">Highlight</button><button type="button" data-annotation="underline">Underline</button><button type="button" data-annotation="strikethrough">Strikethrough</button><label class="annotation-color-label">Color <select id="annotation-color" aria-label="Markup color"></select></label><button type="button" data-annotation="note">Note</button><button type="button" data-annotation="undo">Undo</button><button type="button" data-annotation="export">Export PDF</button></div><p class="annotation-selection" role="status">Select words in the PDF, then choose a markup tool or add a note.</p><form class="annotation-note" hidden><label for="annotation-note-text">Note on selected words</label><textarea id="annotation-note-text" rows="2" maxlength="10000"></textarea><button type="submit">Save note</button><button type="button" data-annotation="cancel">Cancel</button></form>';
  ...
  function controls(){
    ...
    for(const act of TEXT_MARKUP_ACTS)button(act).disabled=busy||!valid;
    color.disabled=busy;
