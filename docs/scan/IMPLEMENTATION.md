...
Result fields: `sessionId:string`, `sourceRevision:number`, `pdfBytes:Uint8Array`, `title:string`, `mime:'application/pdf'`, `sourceAssets:Array`, `ocr:{status,pages}`, `timings:Array<{stage,pageId?,ms}>`, `diagnostics:Array<{code,pageId?}>`. Session/revision are captured from the owned checkpoint at finish start, not reread after export. Use these plus an OCR-content fingerprint for idempotent host saves; PDF metadata/hash alone is not a stable scan identity.
...
`native/ios/ScanOCR.swift` uses Vision on an already-upright corrected image and returns text/confidence with normalized top-left-origin bounds. Call it off the UI thread. `OCRFixture.swift` exercises actual macOS Vision on a synthetic English image. That is native OCR execution evidence, not mobile-camera accuracy or integration evidence. JavaScript OCR remains an injected provider; native app bridge is not wired here.

## Searchable output contract

An injected OCR provider can return `{text,lines:[{text,bounds:[x,y,width,height]}]}` using normalized top-left-origin coordinates of the **processed** page image. Valid positioned printable-ASCII text, whose joined lines match the full text, is written as invisible PDF text. Tests compare extracted text and exact rendered pixels against the image-only PDF. Unicode/non-Latin scripts, unpositioned text and invalid bounds remain sidecars with `OCR_NOT_EMBEDDED`; no silently corrupt text layer is substituted. Current searchable statuses are `searchable` (all pages), `partially-searchable`, `sidecar-only`, `partial` (some OCR results but no embedded layer), `failed`, and `unsupported` (no provider). This is an output capability, not a claim that browser OCR ships with a working recognition model. Provider wiring remains an app-integration requirement.
...
- Follow-up searchable/revision suite: 27 scanner tests passed sequentially. Printable-ASCII text extraction and pixel equality, invalid bounds, unsupported-script sidecar and stable session/revision tests pass. Independent reviewer cleared this increment. A further browser regression caught auto-capture remaining latched when enabled after manual preview; resetting its gate on mode change fixes this, and the synthetic live-canvas capture test passes. Camera fixtures are generated, never a real handset camera.
