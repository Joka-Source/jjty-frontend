...
### 9 September — durable text markup

The own Annotate toolbar now provides Highlight, Underline and Strikethrough with five named colors. The existing operation registry, exact anchors, records, commit-before-paint, undo and reload paths retain actual operation identity and style. Shared color/opacity values drive screen and native PDF output. Backup validation preserves color; normalized single-block ranges retain strict source identity. Organize includes underline-only work in its saved-work copy.

Browser coverage uses DOM Range selection helpers and actual toolbar clicks, then exercises overlapping undo, rejected persistence without paint, cross-page underline, zoom/reload, native PDF.js subtype/color/quads readback and backup restoration into an empty library before repaint/export. A separate underline-only Organize → Save copy → reopen test verifies the original bytes are unchanged. JS/WASM simulated-transcript journeys verify exact targeting and durable underline/strikethrough receipts; unsupported speech cannot silently become Highlight. These do not prove physical dragging, real microphone quality, native adapters or PDF Expert parity.

Desktop and phone screenshots and Poppler-rendered exported pages were inspected. The narrow-screen controls remain too tall: the redundant legacy export panel pushes the document below about 708px in a 390×844 viewport. Execute `tasks/reader-compact-tools-plan.md` next; absence of horizontal overflow is not sufficient UI quality. Canonical receipt records the completed full gate and local revision.

### 9 September — compact own-reader controls

The original annotation export button and status now live in the Annotate toolbar. Its existing snapshot/review controller remains authoritative, with visible errors, explicit export busy state and dirty-note protection. The empty legacy panel is removed from the active workspace; annotated review explains that form answers are included through Fill or Organize.

Narrow screens use an out-of-flow native View popover for existing zoom, page navigation and document/tool controls, with Find and Pages always exposed. Controls retain DOM identity and handlers; wider layouts restore their original positions. Source/route changes close the popover. The page label comes from actual navigation state rather than editable input. Selection survives focus entering reader controls.

Breakpoint changes preserve the last owned reading place through the reader queue. Duplicate layout requests do not schedule old snapshots twice; every resize invalidates an in-flight Fit so final geometry matches the latest viewport. Source ownership and draft guards remain in force. Browser acceptance holds a fit across successive widths and queues a page jump, then verifies final fit and the exact Return departure. Screenshots exposed a portrait-tablet overlap missed by horizontal-overflow checks; compact presentation now extends through 900px, with 44px targets. Final evidence and full gate are recorded in the canonical SSOT receipt.

## Own PDF text Edit workspace — PASS_LOCAL, 9 September

JETT Edit now uses the pinned Bento engine beneath its own page/paragraph controls. A source-bound draft records original text, box and rotation; it is recoverable after reload and through backup. Transactional draft writes preserve concurrent form/metadata saves and cannot be resurrected by a stale whole-document write after discard. Review composes the latest saved marks/forms, rechecks the exact target and opens the existing reviewed-copy Library path. Original bytes and original records remain unchanged; derived copies receive independent source identity.

Preservation checks independently parse/render the produced PDF, compare surrounding text and pixels, page geometry, native annotation/form/link semantics, and original/proposed edit bounds. Protected intersections and unsupported restricted documents are refused. Glyph coverage is checked against actual font bytes including raw CFF; unchanged style islands survive separated text replacements through a bounded edit diff. Tagged/signed/encrypted documents, general layout/font/image editing and scanned OCR text editing are not established by this increment.

Focused evidence covers actual replacement and longer text, mixed styles including two separated edits around unchanged bold text, native cleanup, failed-write retry, stale route/source cancellation, saved outside highlights, intersecting mark/link refusal, draft backup custody, and Edit → reload → Review → Save copy → reload. Desktop/phone and Poppler output have been inspected. The paragraph-picker form remains interim; the next planned increment is direct on-page targeting and editor placement in `tasks/reader-edit-targeting-plan.md`. Full PDF Expert/Apple Books/native-device parity remains open. Final build and all 441 tests pass (235104.043667ms); evidence and the scoped commit receipt are recorded in the canonical human repo under evidence/2026-09-09/own-edit. No push, deployment, real-microphone or native-device claim.


## Direct Edit paragraph targeting — PASS_LOCAL, 9 September

Accessible targets now sit on the existing PDF canvas, with pointer and paragraph-selector routes using the same recoverable draft. Desktop editing sits beside the document; the phone bottom panel keeps the selected text and Review/Discard visible. Main owns workspace geometry changes through its reader queue, preserving page/offset/custom zoom and Return. Targets clear on replaced canvases and stale source/route work; restored targets require original text/box/rotation.

A real rotated/cropped/UserUnit fixture exposed that Bento paragraph boxes use a rotated local frame. The adapter now converts native quadrilaterals explicitly. Actual PDFs at 0/90/180/270 and editable 45-degree text pass selection, edited output validation and saved-copy readback. Clipped hit areas exclude empty bounding-box corners. Independent text/pixel verification uses the union of original/proposed polygons with existing 2-point padding; collateral pixels and text inside the old rectangle but outside the polygon are rejected. Native annotation/link/widget intersections and prior custody gates remain enforced.

Build and all 450 tests pass (247956.295791ms). Actual pointer/zoom/reload, held-fit breakpoint and route cancellation journeys pass; desktop and compact phone output inspected. Evidence is recorded in the canonical human repository at evidence/2026-09-09/direct-edit-targets. No physical Pencil/onscreen-keyboard/native-device, push or deployment claim. Remaining PDF Expert scope remains active; next broaden EPUB reading under tasks/epub-reader-plan.md and the founder's Apple Books quality floor.
