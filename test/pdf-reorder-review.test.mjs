...
test('reorder review validates input, renders a native copy, and downloads its exact bytes',{timeout:60000},async t=>{
 ...
 const page=await browser.newPage();await page.setRequestInterception(true);page.on('request',r=>r.url()===url+'/__reorder.html'?r.respond({status:200,contentType:'text/html',body:html}):r.continue());
 await page.goto(url+'/__reorder.html');await page.waitForFunction(()=>window.ready);
 assert.equal(await page.$eval('#pdf-review-order',n=>n.value),'1-2');
 async function order(value){await page.$eval('#pdf-review-order',(n,v)=>{n.value=v;},value);await page.click('.pdf-review-order button');}
 await order('1,1');assert.equal(await page.$eval('#pdf-review-order',n=>n.getAttribute('aria-invalid')),'true');
 assert.equal(await page.$eval('#pdf-review-download',n=>n.disabled),false);
 await order('2,1');await page.waitForFunction(()=>document.getElementById('pdf-review-title').textContent==='Review reordered original copy'&&!document.getElementById('pdf-review-download').disabled);
 assert.equal(await page.$eval('#pdf-review-order',n=>n.value),'1-2');assert.equal(await page.$$eval('#pdf-review-pages canvas',n=>n.length),2);
