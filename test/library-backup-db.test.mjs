...
test('actual IndexedDB snapshot and add-only restore are atomic across all three stores', {timeout:60000},async t=>{
 ...
 const page=await browser.newPage();await page.goto(new URL("/reader/" + new URL(url).search + new URL(url).hash, url).href);await page.waitForFunction(()=>window.__jtApp?.booted);
