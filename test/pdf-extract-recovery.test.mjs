...
// promise is held/rejected, after extraction has already passed native readback.
...
export function createMuPdfProvider(mupdf){const provider=nativeProvider(mupdf);return {async open(bytes){
 ...
 const document={...opened.document,getPage:async n=>{const page=await getPage(n);return id===2&&n===1?{...page,render:()=>({promise:new Promise((resolve,reject)=>{window.rejectExtractedRender=()=>reject(new Error('Injected extracted preview render failure'));})})}:page;}};
...
test('failed extracted preview preserves the prior copy without reviving stale reviews',{timeout:60000},async t=>{
 const server=spawn(process.execPath,[path.join(root,'node_modules/vite/bin/vite.js'),'--host','127.0.0.1','--port','4986','--strictPort'],{cwd:root,stdio:'ignore'});
 ...
 async function pageFor(t){const page=await browser.newPage();t.after(()=>page.close());await page.setRequestInterception(true);page.on('request',r=>r.url()===url+'/__extract_recovery.html'?r.respond({status:200,contentType:'text/html',body:html}):r.url()===url+'/src/pdf-engine.js'?r.respond({status:200,contentType:'text/javascript',body:engine}):r.continue());await page.goto(url+'/__extract_recovery.html');await page.waitForFunction(()=>window.ready);assert.equal(await page.$eval('#pdf-review-download',n=>n.disabled),false,await page.$eval('#pdf-review-status',n=>n.textContent));await page.$eval('#pdf-review-extract',n=>{n.value='3,1';});await page.click('.pdf-review-extract button');await page.waitForFunction(()=>window.rejectExtractedRender);return page;}
 await t.test('render failure restores original DOM, filename and exact downloadable bytes',async t=>{
  const page=await pageFor(t);await page.evaluate(()=>rejectExtractedRender());
  ...
  assert.equal(await page.$$eval('#pdf-review-pages figure',n=>n.length),3);
  assert.equal(await page.$eval('#pdf-review-print',n=>n.disabled),false);
  assert.equal(await page.$eval('#pdf-review-extract',n=>n.disabled),false);
  assert.equal(await page.$eval('#pdf-review-extract',n=>n.value),'3,1');
 ...
 await t.test('close during failed render never restores the abandoned copy',async t=>{
  const page=await pageFor(t);await page.click('#pdf-review-close');await page.evaluate(()=>rejectExtractedRender());
  await page.waitForFunction(()=>destroyed.includes(2));
 ...
 await t.test('newer review survives the older render failure and downloads its own snapshot',async t=>{
  ...
  await page.evaluate(async()=>{const {rotatePdfPages}=await import('/src/pdf-rotation.js');window.newerBytes=await rotatePdfPages(original,[{pageIndex:1,quarterTurns:1}]);await review.open(newerBytes,'newer.pdf',{kind:'newer'});rejectExtractedRender();});
  await page.waitForFunction(()=>destroyed.includes(2));
