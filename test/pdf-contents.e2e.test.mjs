...
import {spawn} from 'node:child_process';import {readFile,writeFile,mkdtemp,rm} from 'node:fs/promises';import {tmpdir} from 'node:os';import path from 'node:path';import * as mupdf from 'mupdf';import puppeteer from 'puppeteer-core';import {root} from './validate.mjs';
...
test('real PDF contents navigation keeps marks, returns to reading place and survives reopening',{timeout:90000},async t=>{
 ...
 for(const engine of ['mupdf','pdfjs']){
  ...
  await page.click('#pdf-contents-back');assert.equal(await page.evaluate(()=>window.__jtApp.currentBlock()),0);
 ...
 assert.ok(mark.length);await page.reload();await page.waitForFunction(()=>window.__jtApp?.booted&&document.querySelector('#pdf-contents-list .pdf-contents-target'));assert.equal(await page.evaluate(()=>window.__jtApp.currentBlock()),chapterBlock);
