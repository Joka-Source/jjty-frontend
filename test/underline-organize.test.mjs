...
test('underline-only work survives Organize and Save copy to Library without changing the source',{timeout:90000},async t=>{
 ...
 await page.evaluateOnNewDocument(()=>{localStorage.setItem('jt.welcomed','1');localStorage.setItem('jt.mic','off');});await page.goto(new URL("/reader/" + new URL(url).search + new URL(url).hash, url).href);await page.waitForFunction(()=>window.__jtApp?.booted);
