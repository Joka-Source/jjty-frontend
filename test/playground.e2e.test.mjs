...
test('playground supports discovery, review persistence, source inspection and live editor across viewports', {timeout:90000}, async t=>{
 ...
 await page.$eval('[data-code]',button=>button.scrollIntoView({block:'center',behavior:'instant'}));await page.locator('[data-code]').click();await page.waitForSelector('dialog pre code').catch(async error=>{throw new Error(error.message+' '+JSON.stringify(await page.evaluate(()=>({dialog:document.querySelector('dialog')?.textContent,open:document.querySelector('dialog')?.open,source:document.querySelector('[data-code]')?.dataset.code}))));});assert.ok((await page.$eval('dialog pre code',n=>n.textContent)).length>100);await page.keyboard.press('Escape');
