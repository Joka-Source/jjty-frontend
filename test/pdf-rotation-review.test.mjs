...
test('failed rotated preview preserves the prior copy without reviving stale reviews',{timeout:60000},async t=>{
 const server=spawn(process.execPath,[path.join(root,'node_modules/vite/bin/vite.js'),'--host','127.0.0.1','--port','4974','--strictPort'],{cwd:root,stdio:'ignore'});
 ...
 async function pageFor(t){const page=await browser.newPage();t.after(()=>page.close());await page.setRequestInterception(true);page.on('request',r=>r.url()===url+'/__rotation_review.html'?r.respond({status:200,contentType:'text/html',body:html}):r.url()===url+'/src/pdf-engine.js'?r.respond({status:200,contentType:'text/javascript',body:engine}):r.continue());await page.goto(url+'/__rotation_review.html');await page.waitForFunction(()=>window.ready);assert.equal(await page.$eval('#pdf-review-download',n=>n.disabled),false,await page.$eval('#pdf-review-status',n=>n.textContent));await page.click('[data-pdf-rotate="right"]');await page.waitForFunction(()=>window.rejectRotatedRender);return page;}
