...
test('atomic rename preserves document data and survives queued stale form/server writes', {timeout:60000},async t=>{
 ...
 const browser=await puppeteer.launch({executablePath:process.env.CHROME_PATH??'/Applications/Google Chrome.app/Contents/MacOS/Google Chrome',headless:true,args:['--no-first-run']});t.after(()=>browser.close());const page=await browser.newPage();await page.goto(new URL("/reader/" + new URL(url).search + new URL(url).hash, url).href);await page.waitForFunction(()=>window.__jtApp?.booted);
