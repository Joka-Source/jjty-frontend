...
test('JETT visual page overview reorders, extracts and undoes the exact reviewed copy',{timeout:60000},async t=>{
 ...
 if(process.env.JETT_OVERVIEW_EVIDENCE_DIR){await mkdir(process.env.JETT_OVERVIEW_EVIDENCE_DIR,{recursive:true});for(const [name,width,height] of [['desktop',1280,1000],['phone',390,844]]){await page.setViewport({width,height});await page.screenshot({path:path.join(process.env.JETT_OVERVIEW_EVIDENCE_DIR,`${name}.png`)});}}
});
