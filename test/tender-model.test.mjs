...
import {tender,requirements,initialBid,blockers,fileProblem,journey} from '../workspace/tender-model.js';
...
test('readiness journey keeps government submission separate from preparation',()=>{
 const bid=initialBid();
 assert.deepEqual(journey(bid).stages.map(stage=>stage.complete),[false,false,false,false,false]);
 bid.company='Synthetic society';bid.registration='MSS-A-TEST';bid.eligibility=true;bid.corrigenda=true;
 for(const r of requirements)bid.documents[r.id]={reviewed:true};
 const ready=journey(bid);
 assert.equal(ready.percent,100);assert.equal(ready.current,4);
 assert.deepEqual(ready.stages.map(stage=>stage.complete),[true,true,true,true,false]);
});
