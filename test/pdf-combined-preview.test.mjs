...
test('filled copy can include committed marks with saved answers and preserve the original', {timeout:90000},async t=>{
 ...
 const download=async(label,preview=true)=>{
  ...
  if(preview){await page.locator('#pdf-form-preview').click();await page.waitForFunction(()=>document.getElementById('pdf-review-dialog').open&&!document.getElementById('pdf-review-download').disabled);if(label==='combined')assert.equal(await page.$eval('#pdf-review-title',n=>n.textContent),'Review filled and annotated copy');await page.locator('#pdf-review-download').click();}
