...
test('filled copy can include committed marks with saved answers and preserve the original', {timeout:90000},async t=>{
 ...
 await page.goto(new URL("/reader/" + new URL(url).search + new URL(url).hash, url).href);await page.waitForFunction(()=>window.__jtApp?.booted);await clickReaderControl(page,'#welcome-next');await clickReaderControl(page,'#welcome-skip');
