...
test('notebook import survives reload and supports page order, zoom and narrow layout', {timeout:90000}, async t => {
  ...
  await (await page.$('#pdf-file')).uploadFile(path.join(root,'test/fixtures/jett-fillable.pdf'));
  ...
  assert.equal(await page.$eval('#ink image',n=>n.getAttribute('href')),first);
  ...
  assert.equal(await page.$eval('[aria-label="Document outline"]',n=>n.textContent),'Review form');
  await page.click('#undo');
  await page.click('[data-page="0"]');
  assert.equal(await page.$eval('#ink image',n=>n.getAttribute('href')),first);
  await page.select('#zoom','2');
  assert.equal(await page.$eval('.paper',n=>n.getBoundingClientRect().width),1440);
  await page.setViewport({width:390,height:844});
  assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth),390);
  await page.select('#zoom','0');
  assert.ok(await page.$eval('.paper',n=>n.getBoundingClientRect().width<390));
});
