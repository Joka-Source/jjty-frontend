...
test('a bookmark selected while saved reading position returns keeps the newer place',{timeout:90000},async t=>{
  ...
  if(!await page.$eval('#reader-page-browser',n=>n.open))await page.click('#reader-pages-toggle');
  await page.waitForFunction(()=>document.getElementById('reader-page-browser').matches(':modal'));
